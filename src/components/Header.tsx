...
export default function Header() {
  ...
  // Lock body scroll when mobile menu is open; auto-restore on unmount
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Mobile Full-Screen Menu Overlay — rendered as a sibling OUTSIDE <header>
          so `fixed` positioning is relative to the viewport, not the sticky element */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-white dark:bg-slate-950 flex flex-col pt-24 px-6 md:hidden overflow-y-auto pb-10">
          {/* Close button positioned in same location as hamburger */}
          <button
            className="absolute top-5 right-6 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white p-2"
          ...
          <nav className="flex flex-col space-y-6 text-2xl font-bold text-slate-800 dark:text-slate-100">
            <Link href="/markets" onClick={closeMobileMenu} className="hover:text-brand transition-colors border-b border-slate-200 dark:border-slate-800 pb-4">Markets</Link>
            <Link href="/platforms" onClick={closeMobileMenu} className="hover:text-brand transition-colors border-b border-slate-200 dark:border-slate-800 pb-4">Platforms</Link>
            <Link href="/academy" onClick={closeMobileMenu} className="hover:text-brand transition-colors border-b border-slate-200 dark:border-slate-800 pb-4">Academy</Link>
            <Link href="/about-us" onClick={closeMobileMenu} className="hover:text-brand transition-colors border-b border-slate-200 dark:border-slate-800 pb-4">About Us</Link>
          </nav>
